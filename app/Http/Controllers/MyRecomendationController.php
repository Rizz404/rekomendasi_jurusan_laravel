...
class MyRecomendationController extends Controller
{
    ...
    {
        $user = Auth::user();
        // Pastikan user memiliki relasi student atau handle jika tidak ada
        $student = $user->student()->firstOrFail(); // Lebih singkat jika relasi sudah didefinisikan di User model
        ...
        if ($SAWResults->isEmpty())
        {
            ...
            if ($SAWResults->isEmpty())
            {
                ...
                return view('user.my-recommendation.index', compact('SAWResults'))
